...
Moving forward:
Initial relational tables were developed on the back of experience using SQL for mining of large biological datasets. Prior to this, an object-oriented model of strength training was built in C++ for a micro-controller (Arduino) project. The Python code of the Desktop GUI partly originated from the C++ objects and adopted a model-view based structure alongside relational data tables. This was well suited to Qt for Python. Moving to Typescript for the progressive Web App front end, types have been used alongside a mainly functional approach which seems more suited than enforcing strict object orientation. Composables and Pinia are used to help with encapsulation and to provide structure whilst readability is maximised with composition API and setup stores. The data structures continue to evolve to provide the best possible foundation for the UI. For example, database RPE lookup tables were replaced in favour of a (probably more accurate) formula that can easily be implemented in the front end without querying the back end at all.       
